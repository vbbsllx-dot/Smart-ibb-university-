...
export default function ResumeTransaction({ isOpen, onClose, studentData }: TransactionProps) {
  ...
  // 🖨️ دالة الطباعة الخاصة بـ (فتح القيد / استئناف الدراسة)
  const handlePrint = () => {
    ...
    const htmlContent = `
      ...
      <head>
        <title>طباعة استمارة استئناف الدراسة</title>
        <style>
          @media print {
      ...
      <body>
        ...
        <div class="title">استمارة طلب استئناف دراسة (فتح قيد)</div>
        ...
        <div class="content">
          ...
          أتقدم إليكم بطلب <strong>فتح قيدي واستئناف دراستي</strong> بالكلية اعتباراً من العام الجامعي <strong>${academicYear || '................'}</strong>، وذلك بعد انتهاء فترة التوقيف السابقة.<br>
          <strong>ملاحظات إضافية:</strong> ${reason || 'لا يوجد'}<br>
          <strong>والمرفقات:</strong> ${attachmentFile ? attachmentFile.name : 'لا توجد'} لديكم.<br>
          وأرجو التكرم بالموافقة على استئناف قيدي وإبلاغ شؤون الطلاب بذلك لتسجيلي ضمن كشوفات العام الحالي.
        </div>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (attachmentFile) {
        ...
        const fileName = `resume_${academicId}_${Date.now()}.${fileExt}`;
      ...
      const { error: insertError } = await supabase
        ...
        .insert([{
          ...
          transaction_type: 'resume', // 👈 نوع المعاملة: استئناف/فتح قيد
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-300 rtl:text-right ltr:text-left dir-auto">
      <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 max-h-[90vh] flex flex-col">
        ...
        {/* الترويسة بلون أخضر زمردي مميز */}
        <div className="bg-emerald-500 p-5 text-white flex justify-between items-center shrink-0">
          <div>
            <h3 className="font-black text-lg">{t('modalResumeTitle')}</h3>
            <p className="text-xs text-emerald-100 mt-1">{t('modalResumeSubtitle')}</p>
          </div>
        ...
        <div className="p-6 overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="text-[11px] font-black text-slate-700 block mb-1.5">{t('lblDeanName')} <span className="text-emerald-500">*</span></label>
                <input 
                  type="text" required value={deanName} onChange={(e) => setDeanName(e.target.value)} placeholder={t('lblDeanPlaceholder')}
                  className="w-full bg-white border border-slate-200 text-slate-800 font-medium p-2.5 rounded-xl focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 transition-all text-xs outline-none" 
                />
              </div>

              <div>
                <label className="text-[11px] font-black text-slate-700 block mb-1.5">{t('lblAcademicYear')} <span className="text-emerald-500">*</span></label>
                <input 
            ...
            <div>
              <label className="text-[11px] font-black text-slate-700 block mb-1.5">{t('lblResumeReason')} <span className="text-emerald-500">*</span></label>
              <textarea 
                required rows={3} value={reason} onChange={(e) => setReason(e.target.value)} placeholder="اذكر أي ملاحظات إضافية حول عودتك للدراسة..."
                className="w-full bg-white border border-slate-200 text-slate-800 font-medium p-2.5 rounded-xl focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 transition-all text-xs outline-none resize-none" 
              />
            </div>

            <div>
              <label className="text-[11px] font-black text-slate-700 block mb-1.5">{t('lblResumeAttachment')} <span className="text-slate-400 font-normal">{t('lblOptional')}</span></label>
              <input 
                type="file" onChange={(e) => setAttachmentFile(e.target.files ? e.target.files[0] : null)}
                className="w-full bg-white text-slate-600 font-medium p-2 rounded-xl border border-slate-200 text-xs outline-none file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-black file:bg-emerald-50 file:text-emerald-600 hover:file:bg-emerald-100 cursor-pointer" 
              />
            </div>
            ...
            <div className="pt-4 flex gap-3">
              <button 
                ...
                className={`flex-1 font-bold py-3.5 rounded-xl transition-all duration-300 flex items-center justify-center gap-2 shadow-md ${isSubmitting ? 'bg-slate-300 text-slate-500 cursor-not-allowed' : 'bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white hover:shadow-lg'}`}
              >
                {isSubmitting ? t('msgUploading') : t('btnSubmitResume')}
              </button>
