...
export default function SuspendTransaction({ isOpen, onClose, studentData }: TransactionProps) {
  const t = useTranslations('StudentDashboard');

  const [academicYear, setAcademicYear] = useState(new Date().getFullYear().toString());
  const [suspendPeriod, setSuspendPeriod] = useState(t('periodFullYear')); // 👈 حالة جديدة لفترة الوقف
  ...
  // 🖨️ دالة الطباعة الخاصة بـ (وقف القيد)
  const handlePrint = () => {
    ...
    const htmlContent = `
      ...
      <head>
        <title>طباعة استمارة وقف القيد</title>
        <style>
          @media print {
      ...
      <body>
        ...
        <div class="title">استمارة طلب وقف قيد أكاديمي</div>
        ...
        <div class="content">
          ...
          أتقدم إليكم بطلب <strong>وقف قيدي الأكاديمي</strong> مؤقتاً لمدة (<strong>${suspendPeriod}</strong>) اعتباراً من العام الجامعي <strong>${academicYear || '................'}</strong>.<br>
          <strong>وذلك للمبررات التالية:</strong> ${reason || '................................................................................................'}<br>
          <strong>والمرفقات الثبوتية:</strong> ${attachmentFile ? attachmentFile.name : 'لا توجد'} لديكم.<br>
          وأرجو التكرم بالموافقة على الطلب وإبلاغ شؤون الطلاب بذلك ليتم حفظ مقعدي وفقاً للوائح الجامعة.
        </div>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (attachmentFile) {
        ...
        const fileName = `suspend_${academicId}_${Date.now()}.${fileExt}`;
      ...
      const transactionDetails = {
        ...
        suspend_period: suspendPeriod, // 👈 إرسال فترة الوقف
        dean_name: deanName,
        reason: reason
      };
      ...
      const { error: insertError } = await supabase
        ...
        .insert([{
          ...
          transaction_type: 'suspend', // 👈 تغيير نوع المعاملة
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-300 rtl:text-right ltr:text-left dir-auto">
      <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 max-h-[90vh] flex flex-col">
        ...
        {/* ترويسة النافذة بلون رمادي داكن مميز لمعاملة الوقف */}
        <div className="bg-slate-700 p-5 text-white flex justify-between items-center shrink-0">
          <div>
            <h3 className="font-black text-lg">{t('modalSuspendTitle')}</h3>
            <p className="text-xs text-slate-300 mt-1">{t('modalSuspendSubtitle')}</p>
          </div>
        ...
        <div className="p-6 overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="text-[11px] font-black text-slate-700 block mb-1.5">{t('lblDeanName')} <span className="text-slate-700">*</span></label>
                <input 
                  type="text" required value={deanName} onChange={(e) => setDeanName(e.target.value)} placeholder={t('lblDeanPlaceholder')}
                  className="w-full bg-white border border-slate-200 text-slate-800 font-medium p-2.5 rounded-xl focus:border-slate-500 focus:ring-2 focus:ring-slate-200 transition-all text-xs outline-none" 
                />
              </div>

              <div>
                <label className="text-[11px] font-black text-slate-700 block mb-1.5">{t('lblAcademicYear')} <span className="text-slate-700">*</span></label>
                <input 
            ...
            {/* 👈 القائمة المنسدلة لفترة الوقف */}
            <div>
              <label className="text-[11px] font-black text-slate-700 block mb-1.5">{t('lblSuspendPeriod')} <span className="text-slate-700">*</span></label>
              <select 
                required value={suspendPeriod} onChange={(e) => setSuspendPeriod(e.target.value)} 
                className="w-full bg-white border border-slate-200 text-slate-800 font-medium p-2.5 rounded-xl focus:border-slate-500 focus:ring-2 focus:ring-slate-200 transition-all text-xs outline-none cursor-pointer" 
              >
                <option value={t('periodSem1')}>{t('periodSem1')}</option>
                <option value={t('periodSem2')}>{t('periodSem2')}</option>
                <option value={t('periodFullYear')}>{t('periodFullYear')}</option>
              </select>
            </div>

            <div>
              <label className="text-[11px] font-black text-slate-700 block mb-1.5">{t('lblSuspendReason')} <span className="text-slate-700">*</span></label>
              <textarea 
                required rows={3} value={reason} onChange={(e) => setReason(e.target.value)} placeholder="اذكر الأسباب التي دعتك لطلب التوقيف..."
                className="w-full bg-white border border-slate-200 text-slate-800 font-medium p-2.5 rounded-xl focus:border-slate-500 focus:ring-2 focus:ring-slate-200 transition-all text-xs outline-none resize-none" 
              />
            </div>

            <div>
              <label className="text-[11px] font-black text-slate-700 block mb-1.5">{t('lblSuspendAttachment')} <span className="text-slate-400 font-normal">{t('lblOptional')}</span></label>
              <input 
                type="file" onChange={(e) => setAttachmentFile(e.target.files ? e.target.files[0] : null)}
                className="w-full bg-white text-slate-600 font-medium p-2 rounded-xl border border-slate-200 text-xs outline-none file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-black file:bg-slate-100 file:text-slate-700 hover:file:bg-slate-200 cursor-pointer" 
              />
            </div>
            ...
            <div className="pt-4 flex gap-3">
              <button 
                ...
                className={`flex-1 font-bold py-3.5 rounded-xl transition-all duration-300 flex items-center justify-center gap-2 shadow-md ${isSubmitting ? 'bg-slate-300 text-slate-500 cursor-not-allowed' : 'bg-slate-700 hover:bg-slate-800 text-white hover:shadow-lg'}`}
              >
                {isSubmitting ? t('msgUploading') : t('btnSubmitSuspend')}
              </button>
              
              <button 
                type="button" onClick={handlePrint}
                className="flex-1 font-bold py-3.5 rounded-xl transition-all duration-300 flex items-center justify-center gap-2 shadow-md bg-slate-200 hover:bg-slate-300 text-slate-800 hover:shadow-lg"
              >
                {t('btnPrint')}
              </button>
            </div>
