...
export default function AdsSlider() {
  ...
  // 🔄 التبديل التلقائي بين السلايدات كل 6 ثوانٍ
  useEffect(() => {
      ...
      <div className="relative w-full h-[220px] sm:h-[280px] md:h-[320px] rounded-[32px] md:rounded-[40px] overflow-hidden border border-white/80 shadow-[0_20px_50px_rgba(0,0,0,0.12)] bg-[#0A2540] group">
        ...
        {isLoadingAds ? (
          <div className="w-full h-full flex flex-col items-center justify-center text-slate-300 gap-3">
            ...
            <span className="text-xs font-mono font-bold tracking-wider">جاري تحميل البث الأكاديمي...</span>
          </div>
        ) : (
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <img 
                ...
                alt={currentAd.title || "إعلان جامعي"} 
                className="w-full h-full object-cover object-center"
              />

              {/* 🌟 التدرج الظلي الذكي لضمان أعلى نسبة مقروئية */}
              <div className="absolute inset-0 bg-gradient-to-l from-[#061e18]/90 via-[#0A2540]/75 to-transparent/20" />
              
              {/* محتوى الإعلان */}
              <div className="absolute inset-0 flex flex-col justify-between p-6 sm:p-10 md:p-12 text-right">
        ...
        {/* 🏹 أسهم التحكم الجانبية */}
        {ads.length > 1 && !isLoadingAds && (
          <>
            <button 
              type="button"
              onClick={prevSlide}
              className="absolute right-4 sm:right-6 top-1/2 -translate-y-1/2 w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-black/30 hover:bg-black/60 border border-white/20 text-white flex items-center justify-center transition-all duration-300 backdrop-blur-md opacity-0 group-hover:opacity-100 hover:scale-110 active:scale-95 cursor-pointer z-20"
              title="السابق"
            >
            ...
            <button 
              type="button"
              onClick={nextSlide}
              className="absolute left-4 sm:left-6 top-1/2 -translate-y-1/2 w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-black/30 hover:bg-black/60 border border-white/20 text-white flex items-center justify-center transition-all duration-300 backdrop-blur-md opacity-0 group-hover:opacity-100 hover:scale-110 active:scale-95 cursor-pointer z-20"
              title="التالي"
            >
              <ChevronLeft className="w-6 h-6" />
            </button>

            {/* 🔘 مؤشرات النقاط بالأسفل */}
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-2 z-20 bg-black/20 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10">
              {ads.map((_, idx) => (
                <button
                  key={idx}
                  type="button"
